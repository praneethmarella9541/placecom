...
/**
 * Grants/revokes "anyone with the link can edit" sharing on a spreadsheet
 * file, scoped to how long the /sheets tab for it stays open — the
 * iframe-embedded real Google Sheets editor only works without a signed-in
 * Google session in-browser when the file allows anonymous link editing.
 * See app/(workspace)/sheets/[id]/page.tsx for the grant-on-mount /
 * revoke-on-leave lifecycle this pairs with.
 */

function errorResponse(e: unknown, fallback: string, spreadsheetId: string) {
    ...
    return NextResponse.json(
      {
        ...
        message:
          "This sheet is shared with the connected account, but only its owner (or someone they've given sharing rights to) can change who has access — which this editor needs to do. Ask the owner to either share it as \"Anyone with the link can edit,\" or give the connected account permission to manage sharing.",
        webViewLink: `https://docs.google.com/spreadsheets/d/${encodeURIComponent(spreadsheetId)}/edit`,
      },
      { status: 403 }
    );
  }
...
export async function POST(
  ...
  try {
    const existing = await listFilePermissions(auth.accessToken, spreadsheetId);
    const alreadyShared = existing.some((p) => p.type === "anyone");
    if (alreadyShared) {
      return NextResponse.json({ granted: false, alreadyShared: true });
    }

    const permission = await addFilePermission(auth.accessToken, spreadsheetId, {
      role: "writer",
      type: "anyone",
    });
    return NextResponse.json({ granted: true, alreadyShared: false, permissionId: permission.id });
  } catch (e) {
    return errorResponse(e, "Failed to grant link sharing", spreadsheetId);
  }
}
...
export async function DELETE(
  ...
  try {
    await deleteFilePermission(auth.accessToken, spreadsheetId, permissionId);
    return NextResponse.json({ ok: true });
  } catch (e) {
    return errorResponse(e, "Failed to revoke link sharing", spreadsheetId);
  }
}
