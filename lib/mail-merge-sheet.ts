...
function looksLikeName(s: string): boolean {
  ...
  // Letters (ASCII + common Latin extensions), spaces, dot, hyphen, apostrophe.
  // Avoids the `u` regex flag which requires target: es2018+ (tsconfig has no
  // target set, defaulting to es5 — Vercel build would otherwise fail).
  return /^[A-Za-zÀ-ɏ][A-Za-zÀ-ɏ\s'.-]*$/.test(t);
}
...
function finalizeHeaderLabels(headerRow: string[], dataRows: string[][]): string[] {
  const labels = headerRow.map((h) => stripBom(h) || "");
